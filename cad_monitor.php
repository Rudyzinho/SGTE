<?php
	...
	if (isset($_POST['cad_monitor'])) {
		$tel = $_POST['tel'];
		$nome = $_POST['nome'];

				$query = mysqli_query($conn ,"SELECT * FROM monitor WHERE nome_do_motorista = '$nome'");
				if (mysqli_num_rows($query) < 1) {
						$query = mysqli_query($conn ,"SELECT * FROM monitor WHERE telefone = '$tel'");
							if (mysqli_num_rows($query) < 1) {
					mysqli_query($conn, "INSERT INTO monitor(nome_do_motorista, telefone) VALUES ('$nome','$tel')");
						echo "
					...
					<div class=\"alert alert-danger\" role=\"alert\">
					  Telefone do monitor já cadastrado, informação não inserida no banco de dados, por favor insira dados validos e não redundantes.
					</div>";
					}
				}else{
					echo "
					<div class=\"alert alert-danger\" role=\"alert\">
					  Nome do monitor já cadastrado, informação não inserida no banco de dados, por favor insira dados validos e não redundantes.
					</div>
				";
				}
...
<body>
	...
	<header id="header">
		...
		<input type="text" name="nome">
						...
						<ul class="actions">
							<ul class="actions">
							<li><input type="submit" value="Cadastrar" name="cad_monitor" class="alt"></li>
						</ul>
						</div>
				</div>
